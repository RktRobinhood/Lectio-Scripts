...
Every completed repository change must be committed and pushed to GitHub; work is not finished while relevant changes exist only in the local worktree. Every modified userscript must also increment its `@version`, keep its registration version in sync, and update the matching Catalogue version when it is a module — three-part `MAJOR.MINOR.PATCH`, exactly one part per change, never reused and never decreased (see [ADR-0003](./docs/adr/0003-static-catalogue-independent-versioning.md) for which part to bump). When a change resolves a GitHub issue, close that issue as part of the same delivery and reference the resolving commit or explain the resolution in the closing comment.
