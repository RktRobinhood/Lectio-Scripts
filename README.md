...
- opens on a compact **Installed** tab, with a counted **Available** tab for discovering modules you have not installed yet; it recognises whether you are signed in as a student or a teacher and, on Stable, offers only the modules and settings meant for your kind of account (Experimental shows every kind, for testing); its category filter narrows only that available set, and an installed module never appears in both,
